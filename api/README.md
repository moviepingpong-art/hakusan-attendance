...
- ただし**入れている端末からは、あとからでも取り出せる**（「あなたの管理リンク」）。
  合鍵はその端末にしか無く、サーバーは SHA-256 しか持っていないので、これは復旧手段ではない
- `?k=` で来た場合は `#k=` に移し替えてから読む（クエリに残さないため）
- 主催者が選べば、合鍵を**その端末に覚える**（`localStorage.dropperAdminKey`）。
  ドロッパーから `#t=<行事>` で開いたときの取り込み先になる
- 覚えていない端末に届いた行事は `dropperPending` に**1時間だけ預かる**。
  管理リンクで開き直した時点で取り込む（**捨てると読み直しになる**）

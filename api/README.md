...
**いまは段階1まで。** 団体をつくる・名簿・削除まで動く。
行事とイベントと回答は段階2以降なので、**まだ本番に配らないこと。**
...
- 行事（貼り付け取り込み・一覧・出欠を作る）
- イベントと回答（`events` / `event` / `answer` / `summary` / `myanswers` / `whoami` / `register`）
- 主催者の画面（`attend/admin.html`）

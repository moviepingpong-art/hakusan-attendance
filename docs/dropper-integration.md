...
> 旧システムはこの変更が公開された時点で新しい大会を受け取れなくなるが、
> **白山クラブは旧システムを一度も使っていなかったため、実害はなかった**（2026-08-12 に公開ずみ）。
> 白山クラブの立ち上げ手順は [hakusan-setup.md](hakusan-setup.md)。
...
- [ ] **LINEに貼って、出欠リンクが青下線・タップ可になること**（約125字。実測128字・237字は可なので収まるはず）
